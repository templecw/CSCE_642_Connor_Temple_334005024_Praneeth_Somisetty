...
class MyRolloutWorker(RolloutWorker):
    ...
    def __init__(self, env_cls,
        ...
        self.total_steps = 1001 # variable to track when to take TD3 actions
...
class TD3CNNActorCritic(nn.Module):
    """
    ...
    def __init__(self, observation_space, action_space):
        ...
        self.q1 = MyCNNQFunction(observation_space, action_space)
        self.q2 = MyCNNQFunction(observation_space, action_space)
...
# We implement the TD3 algorithm by adapting the OpenAI Spinup implementation.

class TD3TrainingAgent(TrainingAgent):
    """
    ...
    def __init__(self,
                 ...
                 model_cls=TD3CNNActorCritic,  # An actor-critic module, encapsulating our ActorModule
        ...
        self.q_params = itertools.chain(self.model.q1.parameters(), self.model.q2.parameters()) # Group q network parameters together
        self.pi_optimizer = Adam(self.model.actor.parameters(), lr=self.lr_actor)
        self.q_optimizer = Adam(self.q_params, lr=self.lr_critic)
        self.act_limit = action_space.high[0] # Upper action limit for training smoothing
        self.q_up_per_policy_up = 2 # Delayed policy update
        self.i_update = 0 # Total number of updates. 
    ...
    def train(self, batch):
        ...
        self.i_update += 1 # increment total number of updates 
        update_policy = (self.i_update % self.q_up_per_policy_up == 0) # set boolean to see if we should update policy
        # First, we decompose our batch into its relevant components, ignoring the "truncated" signal:
        o, a, r, o2, d, _ = batch

        # We sample an action in the current policy if we are about to update policy:
        if update_policy:
            pi = self.model.actor(obs=o)
        ...
        with torch.no_grad():
            pi_targ = self.model_target.actor(o2) # Get an action from our target policy

            # Epsilon to be added to the action, with some additional noise to scale
            epsilon = torch.randn_like(pi_targ) * 0.2 # Target noise
            # Scale the epsilon to be limited 
            epsilon = torch.clamp(epsilon, -0.5, 0.5) 
            a2 = pi_targ + epsilon # Add noise to the action
            a2 = torch.clamp(a2, -self.act_limit, self.act_limit) # scale the action
            # Get target q values with our noisy action, take the minimum
        ...
        if update_policy:
            ...
            # We only use one critic to estimate the value of the action we sampled
            ...
            # Finally, we update our target model only when we update the policy.
            with torch.no_grad():
                for p, p_targ in zip(self.model.parameters(), self.model_target.parameters()):
                    p_targ.data.mul_(self.polyak)
                    p_targ.data.add_((1 - self.polyak) * p.data)

            ret_dict['loss_actor'] = loss_pi.detach().item()
...
training_agent_cls = partial(TD3TrainingAgent,
                             model_cls=TD3CNNActorCritic,
                             gamma=0.99,
                             polyak=0.995,
                             lr_actor=0.000005, #og: 0.000005 Tweak these a little bit. 
                             lr_critic=0.00003) #og: 0.00003 probably this one more than the other
